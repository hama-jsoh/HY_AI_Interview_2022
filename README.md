...
- 🧐 Determinant가 의미하는 바는 무엇인가?  
  ```
  행렬식이라고 번역되며 사전적 의미는 '결정요인' 이다.
  정사각 행렬에 수를 대응시키는 함수의 한 종류이다.
  즉, 정사각행렬이 나타내는 선형변환이 부피를 확대시키는 정도를 나타낸다.
  
  = 선형변환 될 때 단위 면적이 얼마만큼 늘어나는지를 나타낸다.
  ```
- 🧐 Eigen Vector는 무엇인가?  
  ```
  고유벡터라고 번역되며
  ...
  ```
- 🧐 [Eigen Vector는 왜 중요한가?](https://www.youtube.com/watch?v=HL7lcVj7E7U&ab_channel=%EC%9E%A1%EC%8B%9D%EA%B3%B5%EB%8F%8C%EC%9D%B4)
  ```
  어떤 벡터를 선형변환 할 때, 몇몇 점들 중에서 원점에서부터 멀어지는 방향으로 변환하는 scaling을 하는 방향을 찾는 것이 Eigenvalue 문제를 푸는것인데
  역행렬이 존재할 때 k=0 가정은 모순이고, 역행렬은 존재하지 않아야 한다.
  Eigen value는 유일하게 결정되지만 Eigen vector는 유일하지 않다.
  
  transformation matrix A의 특성(characteristic)을 해석하는데 사용되는데, 이것은 변하지 않는 특성이고 기준선이다.
  A는 시스템을 모델링 하는 행렬이므로 행렬 A의 고유 벡터를 안다는 것은 '시스템의 특성을 아는 것'과 같기 때문에 중요하다.
  또한, 람다를 찾는다면 행렬 A를 통한 행렬곱셈 연산 없이 상수 값을 곱하면 되기 때문에 '계산량 또한 줄일 수 있다.'
  
  A를 Eigen Value와 Eigen Vector로 분해하는 고유값 분해에 활용되며,
  정방행렬 뿐만아니라 mxn행렬도 분해할 수 있는 특이값 분해에도 활용된다.
  ```
- 🧐 Eigen Value란?  
  ```
  고유값이라고 번역되며
...
- 🧐 SVD란 무엇인가?→ 중요한 이유는?  
  ```
  특이값 분해라고 번역되며 Singular Value Decomposition의 약자이다.
  행렬 A가 정방 행렬이 아닌 mxn 행렬이면 특이값 분해를 한다.
  
  행랼 A의 특이값 중 일부인 P개만 사용해서 A를 부분 복원할 수 있고, 이는 특이값 분해의 큰 장점 중 하나다.
  행렬의 핵심 특징을 더욱 쉽게 파악할 수 있다.
  만약 어떤 이미지를 나타내는 행렬의 고유값(or 특이값)과 Eigen Vector 를 알고 있다면, 그 정보만을활용해 이미지도 복원할 수 있는 것이다.
  
  예를 들어, 만약 3x3 행렬을 고유값 분해해서 3개의 고유값이 나왔다고 하면, 가장 중요한 고유값 2개만 골라서 원래 행렬의 차원을 축소 시킬 수도 있고,
  가장 중요한 고유값 2개로 데이터의 복원을 할수도 있는 것이다.
  
  이 같은 행렬분해는 주성분분석(PCA)과 같은 차원 축소 기술에 쓰이기도 하고, 고유값 분해를 통해 n제곱 계산을 완화함으로써, 행렬 계산이 많은 머신러닝과 딥러닝에서의
  계산량을 대폭 줄여주고, 그에 따라 계산 시간 및 자원 사용량이 많이 줄어든다.
  
  + 추천 시스템에도 활용된다.
  ```
- 🧐 [Jacobian Matrix란 무엇인가?](https://www.mathworks.com/help/symbolic/sym.jacobian_ko_KR.html)
  ```
  야코비 행렬이라고 번역되며 비선형 변환을 선형 변환으로 근사 시킨 것이다.
  벡터 함수의 야코비 행렬 = 해당 함수의 편도함수 행렬
  스칼라 함수의 야코비 행렬 = 해당 함수의 기울기의 전치
  스칼라에 대한 야코비 행렬 = 해당 함수의 1계 도함수
  ```
